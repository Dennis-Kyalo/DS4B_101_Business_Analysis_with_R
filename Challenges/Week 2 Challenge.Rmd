...
bike_orderlines_tbl %>% group_by(category_1) %>% 
...
bike_orderlines_tbl %>% group_by(category_2) %>% 
    summarise(Sales = sum(total_price)) %>% 
    ungroup() %>% 
    arrange(desc(Sales)) %>%
...
bike_orderlines_tbl %>% group_by(frame_material) %>% 
    summarise(Sales = sum(total_price)) %>% 
    ungroup() %>% 
    arrange(desc(Sales)) %>%
