...
bike_orderlines_tbl %>% 
    ...
    mutate(blank_inc = model %>% str_to_lower() %>% str_detect("black inc")) %>% 
    group_by(blank_inc) %>% 
    summarize(median = median(total_price)) %>% 
    ungroup() %>% 
    mutate(median = scales::dollar(median, largest_with_cents = 10)) %>% 
    rename(`Black Inc` = blank_inc, `Median Orderline` = median)
...
bike_orderlines_tbl %>% 
    ...
    ungroup() %>% 
    mutate(median_orderline = scales::dollar(median_orderline, largest_with_cents = 10)) %>% 
    rename(`Median Orderline` = median_orderline)
...
bike_orderlines_tbl %>% 
    ...
    select(-matches("model_[0-9]"), -model) %>% 
    ...
    set_names(names(.) %>% str_replace(pattern = "_", replacement = " ") %>% str_to_title)
